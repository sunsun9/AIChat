...
import { Plus, MessageSquare, Trash2, LogOut, Zap, User, PanelLeftClose, Pencil, Check, X, AlertTriangle } from 'lucide-react'
...
function DeleteConfirmModal({ title, onConfirm, onCancel }: DeleteConfirmModalProps) {
  ...
  return (
    ...
    >
      <div
        className="w-[320px] rounded-2xl p-5 shadow-2xl animate-fade-up"
        style={{
      ...
      >
        ...
        {/* 标题 */}
        <h3
          className="text-sm font-display font-bold text-center mb-1"
        ...
        <div className="flex gap-2">
          <button
            ...
            onMouseEnter={(e) => {
              ;(e.currentTarget as HTMLButtonElement).style.borderColor = 'color-mix(in srgb, var(--text-soft) 40%, transparent)'
                ...
                >
                  ...
                  {isEditing ? (
                    ...
                    <>
                      ...
                      {/* 删除按钮 */}
                      <button
                        onClick={(e) => requestDelete(e, conv.id, conv.title)}
                        title="删除"
                        className="flex-shrink-0 p-0.5 rounded opacity-0 group-hover:opacity-100 transition-all duration-150"
                        style={{ color: 'var(--text-faint)' }}
                        onMouseEnter={(e) => {
